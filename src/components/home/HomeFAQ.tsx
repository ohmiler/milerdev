    ...
    >
      {items.map((item, index) => (
        <AccordionItem key={item.q} value={`home-faq-${index + 1}`}>
          <AccordionTrigger className="p-5 text-base leading-7 font-semibold no-underline hover:no-underline sm:p-6">
            <span>{item.q}</span>
          </AccordionTrigger>
          <AccordionContent className="px-2 pb-2 text-sm leading-7 text-muted-foreground sm:px-3 sm:text-base">
            <p>{item.a}</p>
          </AccordionContent>
        </AccordionItem>
      ))}
