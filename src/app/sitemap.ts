...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const publishedBundles = await db.select({ slug: bundles.slug, updatedAt: bundles.updatedAt }).from(bundles).where(eq(bundles.status, 'published'));
  ...
  try {
    const publishedPosts = await db.select({ slug: blogPosts.slug, updatedAt: blogPosts.updatedAt }).from(blogPosts).where(eq(blogPosts.status, 'published'));
